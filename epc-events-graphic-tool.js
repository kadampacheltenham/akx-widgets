...
(function(){
  ...
  var SERIF="600 {s}px Fraunces, Georgia, serif", SANS="600 {s}px Inter, Helvetica, Arial, sans-serif", MONO="{s}px ui-monospace, Menlo, Consolas, monospace";
  function f(t,s){return t.replace("{s}",s);}
  function q(id){return document.getElementById(id);}
  function val(id){return q(id).value.trim();}

  function drawMotif(ctx,W,H,name){
    ...
    ctx.strokeStyle="rgba(255,255,255,.2)";ctx.lineWidth=1.5*200/box*Math.max(1.6,box/700);ctx.lineCap="round";ctx.lineJoin="round";
  ...
  function draw(id,W,H){
    ...
    var t=TYPES[val("egm-type")], label=val("egm-label")||t.label, title=val("egm-title"), date=val("egm-date"), sub=val("egm-sub");
    var pad=Math.round(H*0.075), long=W/H>2.5;
    ctx.clearRect(0,0,W,H);ctx.fillStyle=t.colour;ctx.fillRect(0,0,W,H);
    drawMotif(ctx,W,H,t.motif);
    ctx.fillStyle="#fff";ctx.textBaseline="alphabetic";
    // label top-left
    var ls=Math.round(H*(long?0.05:0.03));ctx.font=f(SANS,ls);
    ctx.save();ctx.globalAlpha=.92;ctx.fillText(label.toUpperCase().split("").join(String.fromCharCode(8202)),pad,pad+ls*0.85);ctx.restore();
    ...
    var metaLines=[date,sub].filter(Boolean), lh=ts*1.12, mh=ms*1.5;
    var total=lines.length*lh+(metaLines.length?ms*0.4+metaLines.length*mh:0);
    var y=H-pad-total+ts*0.9;
    ctx.font=f(SERIF,ts);lines.forEach(function(l){ctx.fillText(l,pad,y);y+=lh;});
    if(metaLines.length){y+=ms*0.4;ctx.font=f(MONO,ms);ctx.save();ctx.globalAlpha=.88;metaLines.forEach(function(l){ctx.fillText(l,pad,y);y+=mh;});ctx.restore();}
  }
