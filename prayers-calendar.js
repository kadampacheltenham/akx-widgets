/* Akanishta — Prayers & Pujas calendar (live Google Calendar feed).
...
(function(){
  ...
  var PRESETS = {
    ...
    // add more pages here later, then set var P below (per-page file).
  };
  ...
  var P = PRESETS.prayers;   // Prayers & Pujas
  ...
  function base(item, cal){
    ...
    var cleanTitle=(item.summary||'(untitled)').replace(/#[A-Za-z]{2,8}\b/g,'').replace(/\s{2,}/g,' ').trim();
    return {id:item.id, calKey:cal.key, color:cal.color, calName:cal.name, pinned:!!cal.pinned,
            title:cleanTitle||'(untitled)', start:startStr, allDay:allDay,
